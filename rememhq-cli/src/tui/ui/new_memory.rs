//! In-TUI New Memory creation modal overlay widget.
...
/// Render the New Memory creation modal dialog overlay.
pub fn draw_new_memory_modal(f: &mut Frame, app: &App, area: Rect) {
    let popup_area = centered_rect(75, 70, area);
    f.render_widget(Clear, popup_area);

    let title = " ➕ Create New Memory Record ";
    ...
    // --- Field 0: Content ---
    let is_field0 = app.new_focus_field == 0;
    ...
    let content_para = Paragraph::new(app.new_content.as_str())
        .block(field0_block)
        .wrap(Wrap { trim: false });
    f.render_widget(content_para, inner_chunks[0]);

    // --- Field 1: Memory Type ---
    let is_field1 = app.new_focus_field == 1;
    ...
    let type_para = Paragraph::new(app.new_type.to_string())
        .block(field1_block)
        .style(
            Style::default()
                .fg(Color::Cyan)
    ...
    // --- Field 2: Importance ---
    let is_field2 = app.new_focus_field == 2;
    ...
    let imp_para = Paragraph::new(app.new_importance.as_str()).block(field2_block);
    f.render_widget(imp_para, inner_chunks[2]);

    // --- Field 3: Tags ---
    let is_field3 = app.new_focus_field == 3;
    ...
    let tags_para = Paragraph::new(app.new_tags.as_str()).block(field3_block);
